...
def rule_of_url_replace(url, type):
    ...
    if type == "css":
        url = has_extend_fix(url, ".css")

    if type == "js":
        url = has_extend_fix(url, ".js")
...
'''
    url要被修改的路径
    e传进要被添加的后缀，仅当url没有后缀是添加 .js 或.css之类的
'''
# 写一个函数，判断url文件是否有后缀
def has_extend_fix(url, e):
...
'''
    ...
    问题：3.在生成脚本文件时，部分没有文件名后缀
        已修复，新增方法has_extend_fix方法，用于为文件添加后缀
'''
...
# As = soup.find("link")
# print As["href"]
# str = "http://fonts.googleapis.com/css?family=Cabin%3A400%2C600%7COpen+Sans%3A400%2C300%2C600&#038;ver=4.5.3"
#
# r = re.search("(^(http://|https://|//)([^/]+))?([^\.\?]*)(\.[^?]*)?(\?.*)?", str)
# print r.group(6)
# print has_extend_fix(str, ".css")